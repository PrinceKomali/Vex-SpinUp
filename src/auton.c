...
void bangbang(int d) {
    while (1) {
        if (cutoff_timer >= d) break;
        int r = rotation_get_velocity(ROTATION);
        int error = FLY_TARGET_A - r;
        if (r > FLY_TARGET_A) current_volt_a = 0;
        else current_volt_a = 12000;
        if(error < 3) {
            cutoff_bool = 1;
            conv_start;
        }
        ...
        delay(20);
        if(cutoff_bool) cutoff_timer += 20;
    }
    cutoff_bool = 0;
    cutoff_timer = 0; 
    conv_stop;
...
void auton_long() {
    // start
    point_in_dir(10, 0.4, 2450);
    bangbang(2000);
    point_in_dir(350, 0.2, 2200);
    delay(200);

    // to 1st roller
    forwards(-100,-100);
    delay(2700);
    point_in_dir(90, 0.2, 2200);
    delay(200);
    ...
    delay(500);
    stop;
    imu_set_heading(GYRO, 180);
    imu_reset(GYRO);
    while(imu_get_heading(GYRO) > 180) {}
    forwards(100,100);
    delay(500);
    point_in_dir(360, 0.2, 2200);
    delay(200);
    point_in_dir(90, 0.2, 2200);
